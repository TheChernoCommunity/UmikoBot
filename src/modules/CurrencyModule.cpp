...
CurrencyModule::CurrencyModule(UmikoBot* client) : Module("currency", true), m_client(client)
{
	...
	RegisterCommand(Commands::CURRENCY_WALLET, "wallet", [this](Client& client, const Message& message, const Channel& channel)
	{
					...
					[this, user, channel, &client, message](const QString& icon)
		{
			...
			QString credits = QString::number(getUserData(guild, user).currency() / 100.0);
	...
	RegisterCommand(Commands::CURRENCY_DAILY, "daily", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		else
		{
            ...
            displayedMessage += "You now have **" + QString::number(todaysReward / 100.0) + "** more " + getServerData(channel.guildId()).currencyName + "s in your wallet!\n";
	...
	RegisterCommand(Commands::CURRENCY_GAMBLE, "gamble", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		if (args.size() == 1)
		{
			...
			embed.setDescription("All you need to do is guess a random number between " + QString::number(config.minGuess) + " and " + QString::number(config.maxGuess) + " (inclusive) and if it is the same as the number I think of, you get **" + QString::number(config.gambleReward / 100.0) + config.currencySymbol + "**!\n\n**What number do you think of?** " + utility::consts::emojis::WE_SMART);
		...
		if (args.size() == 2)
		{
			...
			re = QRegExp("[+]?\\d*\\.?\\d{1,2}");
			if (!re.exactMatch(args.at(1))) {
				client.createMessage(message.channelId(), "You cannot specify more than **2 decimal places**.");
				return;
			}
			...
			if (guildList[channel.guildId()][getUserIndex(channel.guildId(), message.author().id())].currency() - qRound(args.at(1).toDouble() * 100) < debtMax)
			{
				client.createMessage(message.channelId(), "**Nope, can't let you get to serious debt.**");
				return;
			}
			...
			{
				client.createMessage(channel.id(), "You cannot bet an amount more than **" + QString::number(gamblebetMax / 100.0) + config.currencySymbol+"**");
				return;
			}
			...
			serverGamble.betAmount = qRound(args.at(1).toDouble() * 100);
		...
		if (args.size() == 1)
		{
			if (!getServerData(channel.guildId()).isRandomGiveawayDone)
			{
				if (getServerData(channel.guildId()).allowGiveaway)
				{
					...
					embed.setDescription(":drum: And today's FREEBIE goes to **" + message.author().username() + "**! \n\n Congratulations! You just got **"+ QString::number(config.freebieReward / 100.0) + config.currencySymbol +"**!");
	...
	RegisterCommand(Commands::CURRENCY_SET_DAILY, "setdaily", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.dailyReward = args.at(1).toInt() * 100;
			client.createMessage(message.channelId(), "Daily Reward Amount set to **" + QString::number(config.dailyReward / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_SET_PRIZE , "setprize", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.freebieReward = args.at(1).toDouble() * 100;
			client.createMessage(message.channelId(), "Freebie Reward Amount set to **" + QString::number(config.freebieReward / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_SET_GAMBLE_LOSS, "setgambleloss", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.gambleLoss = args.at(1).toInt() * 100;
			client.createMessage(message.channelId(), "Gamble Loss Amount set to **" + QString::number(config.gambleLoss / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_SET_GAMBLE_REWARD, "setgamblereward", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.gambleReward = args.at(1).toInt() * 100;
			client.createMessage(message.channelId(), "Gamble Reward Amount set to **" + QString::number(config.gambleReward / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_RICH_LIST, "richlist", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		else
		{
			...
			for (auto i = 0; i < offset && i < leaderboard.size(); i++)
			{
				...
				QString currency = QString::number(user.currency() / 100.0);
	...
	RegisterCommand(Commands::CURRENCY_DONATE, "donate", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		if (guildList[channel.guildId()][getUserIndex(channel.guildId(), message.author().id())].currency() - args.at(1).toDouble() * 100.0 < 0)
		{
		...
		userCurrency.setCurrency(userCurrency.currency() - args.at(1).toDouble() * 100);

		double donation = (args.at(1).toDouble() * 100.0) / mentions.size();
		QString desc = "<@" + QString::number(message.author().id()) + "> donated **" + QString::number(donation / 100.0) + getServerData(channel.guildId()).currencySymbol + "** to";
	...
	RegisterCommand(Commands::CURRENCY_BRIBE, "bribe", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		int amountToBribe = qRound(args.at(1).toDouble() * 100.0);
		...
		{
			QString maxBribeAmount = QString::number(config.bribeMaxAmount / 100.0);
			QString output = QString(
		...
		{
			QString leastBribeAmount = QString::number(config.bribeLeastAmount / 100.0);
			QString output = QString(
	...
	RegisterCommand(Commands::CURRENCY_SET_MAX_BRIBE_AMOUNT, "setmaxbribeamount", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.bribeMaxAmount = args.at(1).toInt() * 100;
			client.createMessage(message.channelId(), "Bribe Max Amount set to **" + QString::number(config.bribeMaxAmount / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_SET_LEAST_BRIBE_AMOUNT, "setleastbribeamount", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.bribeLeastAmount = args.at(1).toInt() * 100;
			client.createMessage(message.channelId(), "Bribe Least Amount set to **" + QString::number(config.bribeLeastAmount / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_STEAL, "steal", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		int amountToSteal = qRound(args.at(1).toDouble() * 100.0);
		...
		if (roll)
		{
			...
			QString stealAmount = QString::number(amountToSteal / 100.0);
			QString output = QString(
		...
		else
		{
			...
			QString fineAmount = QString::number(amountToSteal * config.stealFinePercent / 10000.0);
			QString victimBonus = QString::number(amountToSteal * config.stealVictimBonusPercent / 10000.0);
	...
	RegisterCommand(Commands::CURRENCY_SET_DAILY_BONUS_AMOUNT, "setdailybonus", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			config.dailyBonusAmount = args.at(1).toInt() * 100;
			client.createMessage(message.channelId(), "Daily Bonus Amount set to **" + QString::number(config.dailyBonusAmount / 100.0) + "**");
		});
	});
	...
	RegisterCommand(Commands::CURRENCY_COMPENSATE, "compensate", [this](Client& client, const Message& message, const Channel& channel)
	{
		...
		{
			auto& config = getServerData(channel.guildId());
			int amt = qRound(args.at(1).toDouble() * 100);
			...
			client.createMessage(message.channelId(), "**Everyone has been compensated with `" + QString::number(amt / 100.0) + config.currencySymbol + "`**\nSorry for any inconvenience!");
		});
	});
}
...
{
	QString creditScore = QString::number(getUserData(guild, user).currency() / 100.0);
	auto& config = getServerData(guild);
...
void CurrencyModule::OnLoad(const QJsonDocument& doc) 
{
	...
	for (auto server : servers)
	{
		...
		for (auto user : users) {
			UserCurrency currencyData {
				user.toULongLong(),
						obj[user].toObject()["currency"].toInt(),
						obj[user].toObject()["maxCurrency"].toInt(),
	...
	if (currenConfigfile.open(QFile::ReadOnly))
	{
		...
		for (const auto& server : servers) {
			...
			config.dailyBonusAmount = serverObj["dailyBonusAmount"].toString("5000").toUInt();
			...
			config.bribeMaxAmount = serverObj["bribeMaxAmount"].toString("15000").toInt();
			config.bribeLeastAmount = serverObj["bribeLeastAmount"].toString("2000").toInt();
