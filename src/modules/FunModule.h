...
private:
	...
	struct PollSettings {
		...
		PollSettings(const PollOptions& op, long long maxvotes, snowflake_t chan, const QString& name, int num, double time, const ServerPolls& pollList, snowflake_t msg) : options(op),
			maxVotes(maxvotes), 
			notifChannel(chan), 
			pollName(name), 
			pollNum(num),
